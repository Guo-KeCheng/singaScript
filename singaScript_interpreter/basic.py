...
class Lexer:
    ...
    def advance(self):
        ...
        next_space = self.text.find(" ", self.pos.idx)
        self.current_token = self.text[self.pos.idx:next_space] if self.pos.idx < len(
            self.text) and next_space < len(self.text) else None

    def advance_till_space(self):
        next_space = self.text.find(" ", self.pos.idx)
        
        for i in range ((next_space - self.pos.idx) + 1):
            self.advance()

    def get_next_two_tokens(self):
        next_space = self.text.find(" ", self.pos.idx)
        next_next_space = self.text.find(" ", next_space+1)
        next_next_next_space = self.text.find(" ", next_next_space+1)
        
        if -1 != next_next_space < len(self.text):
            return self.text[next_space+1:next_next_next_space]
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t':
                ...
                elif self.current_token == 'lousier':
                    token, error = self.make_less_than()
                    if error:
                        return [], error
                    tokens.append(token)
                elif self.current_token == 'better':
    ...
    def make_not_equals(self):
        pos_start = self.pos.copy()
        self.advance_till_space()

        if self.current_token == 'same':
            self.advance_till_space()
            return Token(TT_NE, pos_start=pos_start, pos_end=self.pos), None

        self.advance()
        return None, ExpectedCharError(pos_start, self.pos, "'same' (after 'not')")
    ...
    def make_less_than(self):
        ...
        if self.get_next_two_tokens() == 'or same' and self.current_token == 'than':
            tok_type = TT_LTE
            self.advance_till_space()
            self.advance_till_space()
            self.advance_till_space()
        ...
        if self.current_token == 'than':
            tok_type = TT_LT
            self.advance_till_space()
            return Token(tok_type, pos_start=pos_start, pos_end=self.pos), None
        
        return None, ExpectedCharError(pos_start, self.pos, "Missing 'than' or 'or same'")
...
class Parser:
    ...
    def update_current_tok(self):
        if self.tok_idx >= 0 and self.tok_idx < len(self.tokens):
            ...
            self.next_tok = self.tokens[self.tok_idx+1] if self.tok_idx >= 0 and self.tok_idx + 1 < len(self.tokens) else None
            self.next_next_tok = self.tokens[self.tok_idx+2] if self.tok_idx >= 0 and self.tok_idx + 2 < len(self.tokens) else None
    ...
    # chope expr IDENTIFIER
    def expr(self):
        res = ParseResult()

        # If the first keyword is chope
        if self.current_tok.matches(TT_KEYWORD, 'chope'):
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, 'sekali'):
    ...
    def if_expr(self):
        res = ParseResult()
        all_cases = res.register(self.if_expr_cases('sekali'))
        if res.error:
            return res
    ...
    def if_expr_b(self):
        return self.if_expr_cases('otherwise')

    def if_expr_c(self):
        res = ParseResult()
        else_case = None

        if self.current_tok.matches(TT_KEYWORD, 'bobian'):
            res.register_advancement()
            self.advance()
...
class Number(Value):
    ...
    def anded_by(self, other):
        if isinstance(other, Number):
            return Number(True if self.value and other.value else False).set_context(self.context), None
        else:
            return None, Value.illegal_operation(self, other)

    def ored_by(self, other):
        if isinstance(other, Number):
            return Number(True if self.value or other.value else False).set_context(self.context), None
        else:
            return None, Value.illegal_operation(self, other)
...
global_symbol_table.set("simi", BuiltInFunction.print)
